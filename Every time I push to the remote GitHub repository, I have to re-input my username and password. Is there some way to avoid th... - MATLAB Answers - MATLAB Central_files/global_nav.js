...
GlobalNav.showLoggedInHTML = function(langKey, image, loginDisplayName){
      ...
      var dropdown = $("<ul></ul>").addClass('dropdown-menu').attr('role','menu')
          ...
          .append($("<li></li>").addClass('mwa-nav_logout').html($("<a></a>").attr('href','/login/logout?uri='+ encodeURIComponent(window.location.href)).html(GlobalNav.mwa_dropdown_labels[langKey]["signout"])));
...
GlobalNav.showLoggedOutHtml=function(langKey){
      ...
      var loginLink = $("<a></a>").attr('href', '/login?uri='+ encodeURIComponent(window.location.href)).addClass('mwa-nav_login').attr('title', GlobalNav.mwa_dropdown_labels[langKey]["title"]);
